    ...
    while not done:
        ...
        if total > 0:
            ...
            for a in legal_actions:
                idx = agent.action_to_index(a)
                policy_vec[idx] = 1.0 / len(legal_actions)

        # Apply temperature (stochastic during training)
        if temperature > 0:
            p = policy_vec ** (1.0 / temperature)
            p_sum = p.sum()
            if p_sum > 0:
                p /= p_sum
            else:
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=256,
                        help="Mini-batch size for NN training.")
    parser.add_argument("--max-replay", type=int, default=10000,
                        help="Maximum number of positions to keep in replay buffer.")
    ...
    # adjust number of simulations from CLI
    if hasattr(agent, "num_simulations"):
        agent.num_simulations = args.simulations
    ...
    for episode in range(args.episodes):
        ...
        for obs_i, policy_i, player_i in trajectory:
            if winner is None:
                v = 0.0
            else:
                v = 1.0 if winner == player_i else -1.0

            replay_obs.append(obs_i)
            replay_policy.append(policy_i)
            replay_value.append(v)

        # Bound replay buffer size
        if len(replay_obs) > args.max_replay:
            overflow = len(replay_obs) - args.max_replay
            del replay_obs[:overflow]
            del replay_policy[:overflow]
            del replay_value[:overflow]

        # Train on a random mini-batch from replay
        buffer_size = len(replay_obs)
        if buffer_size == 0:
            continue

        batch_size = min(args.batch_size, buffer_size)
        idxs = np.random.choice(buffer_size, size=batch_size, replace=False)

        batch = {
            "obs": [replay_obs[i] for i in idxs],
            "policy_target": [replay_policy[i] for i in idxs],
            "value_target": [replay_value[i] for i in idxs],
        }
